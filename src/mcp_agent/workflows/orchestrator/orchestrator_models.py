...
class AgentTask(Task):
    ...
    agent: str = Field(
        description="Name of agent that the LLM has access to for this task",
        default_factory=list,
    )
...
class Step(BaseModel):
    ...
    tasks: List[ServerTask | AgentTask] = Field(
        description="Subtasks that can be executed in parallel", default_factory=list
    )
...
class Plan(BaseModel):
    ...
    steps: List[Step] = Field(
        description="List of steps to execute sequentially", default_factory=list
    )
    is_complete: bool = Field(
        description="Whether the overall plan objective is complete", default=False
    )
...
class PlanResult(BaseModel):
    ...
    objective: str = Field(description="Objective of the plan", default="")
    plan: Plan = Field(description="The plan that was executed", default_factory=Plan)

    step_results: List[StepResult] = Field(
        description="Results of executing each step", default_factory=list
    )
    ...
    result: str = Field(
        description="Result of executing the plan", default="Plan completed"
    )
