...
class OrchestratorAgent(BaseAgent):
    ...
    async def _execute_plan(
        ...
        plan_result.result = await self._planner_generate_str(
            synthesis_prompt,
            request_params.model_copy(update={"max_iterations": 1})
        )
    ...
    async def _execute_step(
        ...
        for task in step.tasks:
            ...
            futures.append((task, agent.generate_x([
                PromptMessageMultipart(
                    role="user",
        ...
        for future in futures:
            ...
            try:
                ...
                # Add error result
                task_model = task.model_dump()
                task_results.append(TaskWithResult(
                    description=task_model["description"],
                    agent=task_model["agent"],
    ...
    async def _get_next_step(
        ...
        agents = "\n".join([
            self._format_agent_info(agent_name) 
            for agent_name in self.agents.keys()
        ])
    ...
    def _format_agent_info(self, agent_name: str) -> str:
        ...
        instruction = agent.instruction if hasattr(agent, "instruction") else f"Agent '{agent_name}'"
