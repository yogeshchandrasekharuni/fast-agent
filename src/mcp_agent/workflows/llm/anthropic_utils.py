...
def anthropic_message_to_prompt_message_multipart(
    message: Message,
) -> PromptMessageMultipart:
    """
    Convert an Anthropic Message to a PromptMessageMultipart.
    ...
    for block in message.content:
        if block.type == "text":
            # Check if this is a resource marker in the text
            if block.text and (
                block.text.startswith("[Resource:")
                or block.text.startswith("[Binary Resource:")
            ):
                if "\n" in block.text:
                    header, content_text = block.text.split("\n", 1)
                    if "MIME:" in header:
            ...
            mcp_contents.append(TextContent(type="text", text=block.text))

        elif block.type == "image":
            # Handle image block
            if (
                hasattr(block, "source")
                and hasattr(block.source, "media_type")
                and hasattr(block.source, "data")
            ):
                mcp_contents.append(
                    ImageContent(
                        type="image",
    ...
    return PromptMessageMultipart(role=message.role, content=mcp_contents)
