...
@pytest.mark.asyncio
@pytest.mark.skip(reason="This test actually works - it's catching the error correctly - but pytest can't catch SystemExit exceptions")
...
async def test_single_refinement_cycle(fast_agent):
    ...
    async def agent_function():
        async with fast.run() as agent:
            ...
            refined_message = f"{FIXED_RESPONSE_INDICATOR} This is the refined response with more details."
            ...
            await agent.evaluator._llm.generate_x([Prompt.user(f"{FIXED_RESPONSE_INDICATOR} {final_json}")])
...
async def test_max_refinements_limit(fast_agent):
    ...
    async def agent_function():
        async with fast.run() as agent:
            ...
            await agent.evaluator_max._llm.generate_x([Prompt.user(f"{FIXED_RESPONSE_INDICATOR} {first_eval_json}")])
            ...
            await agent.evaluator_max._llm.generate_x([Prompt.user(f"{FIXED_RESPONSE_INDICATOR} {second_eval_json}")])
            ...
            await agent.evaluator_max._llm.generate_x([Prompt.user(f"{FIXED_RESPONSE_INDICATOR} {third_eval_json}")])
...
async def test_early_stop_on_quality(fast_agent):
    ...
    async def agent_function():
        async with fast.run() as agent:
            ...
            first_eval_json = json.dumps(first_eval)
            await agent.evaluator_quality._llm.generate_x([Prompt.user(f"{FIXED_RESPONSE_INDICATOR} {first_eval_json}")])
            ...
            second_eval_json = json.dumps(second_eval)
            await agent.evaluator_quality._llm.generate_x([Prompt.user(f"{FIXED_RESPONSE_INDICATOR} {second_eval_json}")])
...
async def test_structured_output(fast_agent):
    ...
    async def agent_function():
        async with fast.run() as agent:
            ...
            await agent.evaluator_struct._llm.generate_x([Prompt.user(f"{FIXED_RESPONSE_INDICATOR} {eval_json}")])
