...
async def test_apply_prompt_template_with_multipart():
    ...
    prompt_messages = [
        PromptMessage(
            role="assistant", 
            content=TextContent(type="text", text="I'm an assistant")
        ),
        PromptMessage(
            role="user", 
...
async def test_apply_prompt_template_with_multiple_content():
    ...
    prompt_result = GetPromptResult(
        messages=prompt_messages,
        description="Test prompt with multiple parts"
    )
...
async def test_apply_prompt_template_with_mixed_content():
    ...
    llm._apply_prompt_template_provider_specific = AnthropicAugmentedLLM._apply_prompt_template_provider_specific.__get__(llm, AnthropicAugmentedLLM)
    ...
    with patch('mcp_agent.workflows.llm.anthropic_utils.prompt_message_multipart_to_anthropic_message_param') as mock_convert:
        # Set up the mock to return a properly formatted message
        mock_convert.return_value = {
            "role": "user", 
            "content": [
                {"type": "text", "text": "Look at this image:"}, 
                {"type": "image_url", "source": {"type": "base64", "media_type": "image/png", "data": "base64encodeddata"}}
            ]
        }
...
async def test_apply_prompt_template_with_anthropic():
    ...
    prompt_result = GetPromptResult(
        messages=[
            PromptMessage(
                role="assistant", 
                content=TextContent(type="text", text="I'm Claude")
            ),
            PromptMessage(
                role="user", 
...
async def test_apply_prompt_template_with_openai():
    ...
    llm._apply_prompt_template_provider_specific = OpenAIAugmentedLLM._apply_prompt_template_provider_specific.__get__(llm, OpenAIAugmentedLLM)
    ...
    prompt_result = GetPromptResult(
        messages=[
            PromptMessage(
                role="assistant", 
                content=TextContent(type="text", text="I'm GPT-4")
            ),
            PromptMessage(
                role="user", 
    ...
    with patch('mcp_agent.workflows.llm.openai_utils.prompt_message_multipart_to_openai_message_param') as mock_convert:
        # Set up the mock to return a properly formatted message
        mock_convert.return_value = {
            "role": "user", 
