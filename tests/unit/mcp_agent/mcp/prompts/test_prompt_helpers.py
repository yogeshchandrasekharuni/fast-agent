...
def test_get_text(text_content, image_content, text_resource, blob_resource):
    assert get_text(text_content) == "Hello, world!"
    assert get_text(text_resource) == "Resource content"
    assert get_text(image_content) is None
    assert get_text(blob_resource) is None


def test_get_image_data(text_content, image_content, text_resource, blob_resource):
    ...
    assert get_image_data(text_resource) is None


def test_get_resource_uri(text_content, image_content, text_resource, blob_resource):
    assert get_resource_uri(text_resource) == "file:///example.txt"
...
def test_is_text_content(text_content, image_content, text_resource):
    assert is_text_content(text_content) is True
    assert is_text_content(image_content) is False
    assert is_text_content(text_resource) is False


def test_is_image_content(text_content, image_content, text_resource):
    assert is_image_content(image_content) is True
    assert is_image_content(text_content) is False
    assert is_image_content(text_resource) is False


def test_is_resource_content(text_content, image_content, text_resource):
    assert is_resource_content(text_resource) is True
...
def test_message_content_with_prompt_message(text_content, image_content, text_resource):
    text_msg = PromptMessage(role="user", content=text_content)
    image_msg = PromptMessage(role="user", content=image_content)
    resource_msg = PromptMessage(role="user", content=text_resource)
...
def test_message_content_with_multipart(text_content, image_content, text_resource, blob_resource):
    # Create a multipart message with both text and image content
    multipart_msg = PromptMessageMultipart(
        role="user", content=[text_content, image_content, text_resource, blob_resource]
    )
    ...
    assert text_resource in all_resources
