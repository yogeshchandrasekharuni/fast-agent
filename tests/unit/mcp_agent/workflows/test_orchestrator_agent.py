...
async def orchestrator_fixture():
    ...
    agent1 = MagicMock()
    agent1.name = "agent1"
    agent1.generate_x = AsyncMock(return_value=PromptMessageMultipart(
        role="assistant",
        content=[TextContent(type="text", text="Agent1 response")]
    ))
    
    agent2 = MagicMock()
    agent2.name = "agent2"
    agent2.generate_x = AsyncMock(return_value=PromptMessageMultipart(
        role="assistant",
        content=[TextContent(type="text", text="Agent2 response")]
    ))
    ...
    llm.generate_x = AsyncMock(return_value=PromptMessageMultipart(
        role="assistant",
